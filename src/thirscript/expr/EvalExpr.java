...
public class EvalExpr implements Expr
{
    ...
    public ThObject eval(Map<String, Var> env)
    {
        ...
        if (!(fo instanceof IFunction)) throw new UnsupportedOperationException("Cannot call non-function");
        IFunction f = (IFunction) fo;

        List<ThObject> argvs = new ArrayList<>(args.size());
        args.forEach(e -> argvs.add(e.eval(env)));

        if (!f.can_eval(argvs)) throw new UnsupportedOperationException("Cannot call function " + f + " with " + args);
